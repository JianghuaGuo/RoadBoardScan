...
public final class BeepManager implements MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener {
	 ...
	 * 根据配置播放提示音和震动
	 */
	public synchronized void playBeepSoundAndVibrate(boolean isPass) {
		if (playBeep && mediaPlayer != null) {
			mediaPlayer.reset();
			AssetFileDescriptor file = null;
			try {
				if (isPass){
					file = activity.getResources().openRawResourceFd(R.raw.ticket_pass);
				}else{
					file = activity.getResources().openRawResourceFd(R.raw.ticket_error);
				}
